...
public class ledControl extends ActionBarActivity {
   ...
   // Button btnOn, btnOff, btnDis;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        asdsd=getSharedPreferences("data",MODE_PRIVATE);
        s= asdsd.edit();
    ...
    private void turnOnLed()
    {
        if (btSocket!=null)
        {
            try
            {
                btSocket.getOutputStream().write("600".toString().getBytes());
            Toast.makeText(getApplicationContext(),btSocket.getInputStream().toString(), Toast.LENGTH_LONG).show();
            }
            catch (Exception e)
            {
                msg("Error");
    ...
    private class ConnectBT extends AsyncTask<Void, Void, Void>  // UI thread
    {
        ...
        protected void onPostExecute(Void result) //after the doInBackground, it checks if everything went fine
        {
            ...
            else
            {
                msg("Connected.");
                try {
                    String q=asdsd.getString("un","adas7579");
                    btSocket.getOutputStream().write(q.getBytes());
                }catch (Exception e){}
                isBtConnected = true;
            }
